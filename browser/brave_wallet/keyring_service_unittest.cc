...
class TestKeyringServiceObserver
 ...
 public:
  ...
  bool SelectedAccountChangedFired() { return selected_account_change_fired_; }
  ...
  void Reset() {
    ...
    selected_account_change_fired_ = false;
 ...
 private:
  ...
  bool selected_account_change_fired_ = false;
...
class KeyringServiceUnitTest : public testing::Test {
 ...
 protected:
  ...
  static bool RemoveImportedAccount(KeyringService* service,
    ...
    bool success;
    base::RunLoop run_loop;
    service->RemoveImportedAccount(address,
                                   base::BindLambdaForTesting([&](bool v) {
                                     success = v;
                                     run_loop.Quit();
                                   }));
...
TEST_F(KeyringServiceUnitTest, ImportedAccounts) {
  ...
  for (size_t i = 0;
    ...
    EXPECT_TRUE(GetPrivateKeyForImportedAccount(
        &service, imported_accounts[i].address, &private_key));
  ...
  EXPECT_FALSE(RemoveImportedAccount(&service, "0xxxxxxxxxx0"));
  base::RunLoop().RunUntilIdle();
  ...
  std::string private_key;
  EXPECT_FALSE(GetPrivateKeyForImportedAccount(
      &service, imported_accounts[0].address, &private_key));
...
TEST_F(KeyringServiceUnitTest, ImportedAccountFromJson) {
  ...
  std::string private_key;
  EXPECT_TRUE(GetPrivateKeyForImportedAccount(&service, expected_address,
                                              &private_key));
...
TEST_F(KeyringServiceUnitTest, HardwareAccounts) {
  ...
  new_accounts.push_back(mojom::HardwareWalletAccount::New(
      ...
      mojom::CoinType::ETH));
  new_accounts.push_back(mojom::HardwareWalletAccount::New(
      "0xFIL", "m/44'/461'/0'/0/0", "filecoin 1", "Ledger", "device2",
      mojom::CoinType::FIL));
  new_accounts.push_back(mojom::HardwareWalletAccount::New(
      "0x222", "m/44'/60'/1'/0/0", "name 4", "Ledger", "device1",
      mojom::CoinType::ETH));
