...
void WalletPanelUI::CreatePanelHandler(
  ...
  brave_wallet::TxServiceManagerFactory::BindForContext(
      profile, std::move(tx_service_manager_receiver));
  brave_wallet::TxServiceManagerFactory::BindEthTxServiceProxyForContext(
      profile, std::move(eth_tx_service_proxy_receiver));
