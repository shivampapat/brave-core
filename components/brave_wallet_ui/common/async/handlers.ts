...
handler.on(WalletActions.sendTransaction.getType(), async (store: Store, payload: SendTransactionParams) => {
  ...
  if (isEIP1559) {
    ...
    addResult = await apiProxy.txServiceManager.addUnapprovedTransaction({ ethTxData1559: txData1559, ethTxData: undefined, solanaTxData: undefined, filTxData: undefined }, payload.from)
  } else {
    addResult = await apiProxy.txServiceManager.addUnapprovedTransaction({ ethTxData: txData, ethTxData1559: undefined, solanaTxData: undefined, filTxData: undefined }, payload.from)
  }
...
handler.on(WalletActions.sendERC20Transfer.getType(), async (store: Store, payload: ER20TransferParams) => {
  const apiProxy = getAPIProxy()
  const { data, success } = await apiProxy.ethTxServiceProxy.makeERC20TransferData(payload.to, payload.value)
  if (!success) {
...
handler.on(WalletActions.sendERC721TransferFrom.getType(), async (store: Store, payload: ERC721TransferFromParams) => {
  const apiProxy = getAPIProxy()
  const { data, success } = await apiProxy.ethTxServiceProxy.makeERC721TransferFromData(payload.from, payload.to, payload.tokenId, payload.contractAddress)
  if (!success) {
...
handler.on(WalletActions.approveERC20Allowance.getType(), async (store: Store, payload: ApproveERC20Params) => {
  const apiProxy = getAPIProxy()
  const { data, success } = await apiProxy.ethTxServiceProxy.makeERC20ApproveData(payload.spenderAddress, payload.allowance)
  if (!success) {
    console.log(
...
handler.on(WalletActions.approveTransaction.getType(), async (store: Store, txInfo: BraveWallet.TransactionInfo) => {
  const apiProxy = getAPIProxy()
  await apiProxy.txServiceManager.approveTransaction(BraveWallet.CoinType.ETH, txInfo.id)
...
handler.on(WalletActions.rejectTransaction.getType(), async (store: Store, txInfo: BraveWallet.TransactionInfo) => {
  const apiProxy = getAPIProxy()
  await apiProxy.txServiceManager.rejectTransaction(BraveWallet.CoinType.ETH, txInfo.id)
  await store.dispatch(refreshTransactionHistory(txInfo.fromAddress))
})
...
handler.on(WalletActions.rejectAllTransactions.getType(), async (store) => {
  ...
  state.pendingTransactions.forEach(async (transaction) => {
    await apiProxy.txServiceManager.rejectTransaction(BraveWallet.CoinType.ETH, transaction.id)
  })
  await refreshWalletInfo(store)
})
...
export const fetchSwapQuoteFactory = (
  ...
  const { swapService, ethTxServiceProxy } = getAPIProxy()
  ...
  if (quote.success && quote.response) {
    ...
    if (full) {
      ...
      const { estimation: gasEstimates } = await ethTxServiceProxy.getGasEstimation1559()
...
handler.on(WalletActions.refreshGasEstimates.getType(), async (store) => {
  const ethTxServiceProxy = getAPIProxy().ethTxServiceProxy
  const basicEstimates = await ethTxServiceProxy.getGasEstimation1559()
...
handler.on(WalletActions.updateUnapprovedTransactionGasFields.getType(), async (store: Store, payload: UpdateUnapprovedTransactionGasFieldsType) => {
  ...
  if (isEIP1559) {
    const result = await apiProxy.ethTxServiceProxy.setGasFeeAndLimitForUnapprovedTransaction(
      payload.txMetaId,
  ...
  if (!isEIP1559 && payload.gasPrice) {
    const result = await apiProxy.ethTxServiceProxy.setGasPriceAndLimitForUnapprovedTransaction(
...
handler.on(WalletActions.updateUnapprovedTransactionSpendAllowance.getType(), async (store: Store, payload: UpdateUnapprovedTransactionSpendAllowanceType) => {
  ...
  const result = await apiProxy.ethTxServiceProxy.setDataForUnapprovedTransaction(payload.txMetaId, data)
  if (!result.success) {
    console.error(
      'Failed to update unapproved transaction: ' +
      `id=${payload.txMetaId} ` +
...
handler.on(WalletActions.updateUnapprovedTransactionNonce.getType(), async (store: Store, payload: UpdateUnapprovedTransactionNonceType) => {
  const { ethTxServiceProxy } = getAPIProxy()

  const result = await ethTxServiceProxy.setNonceForUnapprovedTransaction(payload.txMetaId, payload.nonce)
  if (!result.success) {
    console.error(
      'Failed to update unapproved transaction: ' +
      `id=${payload.txMetaId} ` +
...
handler.on(WalletActions.retryTransaction.getType(), async (store: Store, payload: BraveWallet.TransactionInfo) => {
  const { txServiceManager } = getAPIProxy()
  const result = await txServiceManager.retryTransaction(BraveWallet.CoinType.ETH, payload.id)
  if (!result.success) {
    console.error(
...
handler.on(WalletActions.speedupTransaction.getType(), async (store: Store, payload: BraveWallet.TransactionInfo) => {
  const { txServiceManager } = getAPIProxy()
  const result = await txServiceManager.speedupOrCancelTransaction(BraveWallet.CoinType.ETH, payload.id, false)
  if (!result.success) {
    console.error(
...
handler.on(WalletActions.cancelTransaction.getType(), async (store: Store, payload: BraveWallet.TransactionInfo) => {
  const { txServiceManager } = getAPIProxy()
  const result = await txServiceManager.speedupOrCancelTransaction(BraveWallet.CoinType.ETH, payload.id, true)
  if (!result.success) {
    console.error(
