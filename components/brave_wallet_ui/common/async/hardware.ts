...
export async function signTrezorTransaction (
  ...
  const nonce = await apiProxy.ethTxServiceProxy.getNonceForHardwareTransaction(txInfo.id)
  ...
  const result =
    await apiProxy.ethTxServiceProxy.processHardwareSignature(txInfo.id, v, r, s)
  if (!result.status) {
...
export async function signLedgerTransaction (
  ...
  const data = await apiProxy.txServiceManager.getTransactionMessageToSign(BraveWallet.CoinType.ETH, txInfo.id)
  ...
  const { v, r, s } = signed.payload
  const result = await apiProxy.ethTxServiceProxy.processHardwareSignature(txInfo.id, '0x' + v, '0x' + r, '0x' + s)
