...
function TreeNode (props: TreeNodeProps) {
      ...
      requestAnimationFrame(() => {
        ...
        const finalValues = els.map((el: HTMLElement) => {
          const rect = getRelativeBoundingRect(svgBoxRect, el)
           // When an element expands, the box height increases and elYCenter gets recalculated. We should always align the tick position for the to the center of the first line.
        ...
        setAxisLeftHeight(
          (Math.min(finalValues[finalValues.length - 1] + 1),
  ...
  const renderTreeExpandToggle = () => {
    return (
      ...
      >
        {isExpanded ? (
          <svg width="12" height="5" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1.5 1h9" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
        ) : (
  ...
  // We're using layout effect here so the initial frame doesn't render an empty space for the axis i.e flash of empty content
